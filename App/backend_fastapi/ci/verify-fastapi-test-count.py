...
# 2026-08-02 기준 실행 건수는 629다(통합 테스트 2파일 제외).
#
# CI 범위를 tests/llm_rag_assistant 에서 tests 전체로 넓혔다. 넓히기 전에는 631건 중 397건만
# 돌아 234건이 아무도 안 보는 상태였다.
#
# 여유를 두지 않고 실제 건수에 맞춘다. 여유를 두면 파일 하나가 통째로 수집 실패해도 통과해
# 커버리지가 소리 없이 사라진다 - 이 스크립트가 막으려는 상황 그 자체다.
# 여기 걸리면 숫자부터 낮추지 말고 왜 줄었는지 먼저 확인한다.
MINIMUM_TESTS = 629
