...
이 기능이 얼마나 정확한지는 [AI 분석 품질을 숫자로 관리합니다](#ai-분석-품질을-숫자로-관리합니다)에
정답 평가셋과 측정값으로 정리해 두었습니다.
...
"AI가 요약해 준다"는 기능은 잘 되는지 눈으로 확인하기 어렵습니다. 그래서 정답을 붙인
회의록 평가셋 30건과 채점기를 만들어, 프롬프트를 고칠 때마다 좋아졌는지 나빠졌는지를
그날 확인할 수 있게 했습니다.

### 두 축으로 나눠 재고, 곱으로 합칩니다

| 축 | 재는 것 | 방법 |
| --- | --- | --- |
| **충실도** | 담아야 할 사실을 담았는가 | 케이스마다 결정·실행항목·배경을 끊어 물음 (문항 39개) |
| **안전성** | 원문에 없는 것을 지어내지 않았는가 | 지어낸 날짜는 문자열 대조로 결정적 검사 |
| **To-Do 추출** | 할 일을 빠짐없이, 없는 건 안 만드는가 | 근거 문장으로 짝지어 F1·재현율 |

평균이 아니라 곱으로 합칩니다. 지어낸 요약은 아무리 많이 담아도 쓸 수 없기 때문입니다.

### 대조군을 항상 같이 잽니다

`rule_based` 티어는 프롬프트를 타지 않습니다. 프롬프트를 고쳤는데 이쪽 점수가 움직이면
고친 게 대상이 아니라 채점기라는 뜻입니다. 실제로 이 신호로 채점기 결함을 다섯 번 잡았습니다.

가장 컸던 건 채점 모델이 요약 대신 회의록 원문을 읽고 답하던 것입니다. 요약만 보고
판정하게 바꾸자 대조군 점수가 **0.649에서 0.056으로** 떨어졌습니다. 그동안 좋아 보이던
점수의 대부분이 채점기가 채워준 것이었습니다.

### 그래서 드러난 진짜 버그

| 문제 | 결과 |
| --- | --- |
| 응답 토큰 상한에 걸려 JSON이 끊겨 분석이 통째로 실패 | 실패 12건 중 1건 → **0건** |
| 폴백 코드 한 줄이 모델의 정답을 덮어씀 | 추출 F1 0.833 → **0.889** (재현율 0.931) |
| 같은 규칙이라도 적는 위치에 따라 작은 모델이 못 읽음 | ollama 충실도 0.151 → **0.496**, huggingface 0.808 유지 |

자세한 측정 과정과 실패한 시도는 [회의록 분석 품질 개선 기록](document_박지수/회의록_분석_품질_개선.md)에
있습니다.
...
| [폴백이 측정에 섞여 유령 0점](docs/trouble-shooting/2026-08-17-폴백이-측정에-섞여-유령-0점이-나온-문제.md) | 정답 4건짜리가 F1 0.000 — 모델은 맞게 뽑고 있었고, 그 순간의 호출만 실패해 규칙 기반 폴백이 답을 만들고 있었다 |
| [채점 문항이 회의록에 따라 답을 뒤집음](docs/trouble-shooting/2026-08-17-요약-안전성-문항이-원문에-따라-답을-뒤집은-문제.md) | 판단이 필요 없는 걸 모델에 물어서 생긴 문제 — 문자열 대조로 옮겨 결정적으로 만들었다 |
...
| **박지수 (부팀장)** | 회의록 AI 분석 — Redis 큐 기반 비동기 처리, LLM 요약 / To-Do 자동 생성 / 분석 품질 평가 체계 |
...
맡은 영역은 실시간 알림(SSE), 회의록 AI 분석 파이프라인, 분석 품질 평가 체계,
