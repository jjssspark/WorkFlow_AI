...
### 5. 산출물 생성 — 범위 제외 (UI 프로토타입)

> **최종 범위에서 제외된 기능입니다.** `/deliverables` 화면은 열리지만 목업 데이터로만 동작하고, 생성 API는 없습니다.

회의록·To-Do·업무 보드 데이터를 참고 자료로 골라 발표자료·보고서·README·제안서 등의 초안을 생성한다는 구상이었습니다.
발표 유형, 발표 시간, 슬라이드 수, 발표 대상 같은 조건 지정 UI까지 만든 뒤, 우선순위 P2 기준에 따라 구현을 다음 단계로 미뤘습니다.

![산출물 생성 화면 (목업)](docs/screenshots/04-deliverables.jpg)
...
- **GitHub 연동** — *미구현.* 저장소 커밋 기록을 기여도 근거로 쓰려던 기능이나, 동기화 로직이 없어 항상 0건입니다
...
| 항목 | 상태 | 실제 동작 |
| --- | --- | --- |
| **GitHub 연동** | 미구현 | `github_records` 테이블과 조회 경로만 존재. 동기화 로직이 없어 심사자 화면에 항상 0건으로 표시됨. `GithubView` 화면은 라우터에 연결되어 있지 않음 |
| **산출물 생성** | 범위 제외 (P2) | `/deliverables` 화면이 목업 데이터로만 동작. 백엔드는 엔티티·리포지토리만 있고 생성 API 없음 |
