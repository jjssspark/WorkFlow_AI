...
이관하면 Supabase의 관리형 백업이 사라지므로 컷오버보다 먼저 만들었다. 컷오버 전인
지금도 운영 데이터(Supabase)를 받고 있다.

- 스크립트: `App/scripts/pg-backup.sh` → 서버 `~/bin/pg-backup.sh`
- 크론: `30 18 * * *` (서버 UTC = **03:30 KST**), 로그 `~/backup/cron.log`
- 보관: 로컬 14일 / 오브젝트 스토리지 60일
- 원격 경로: `s3://workflow-ai-storage/db-backups/<supabase|self-hosted>/`

**컷오버 때 스크립트를 고칠 필요가 없다.** 접속 정보를 파일에 복사해 두지 않고 실행 중인
컨테이너의 환경변수에서 그때그때 읽으므로, 앱이 보는 DB를 그대로 따라간다. 대상이
바뀌면 원격 경로의 `supabase/`가 `self-hosted/`로 자동으로 갈린다.

### 검증한 것

| 항목 | 결과 |
|---|---|
| 실제 백업 1회 | 26초, 1.8MB, 테이블 32개 |
| 덤프 목차 열림 확인 | 스크립트가 매번 `pg_restore -l`로 검사 |
| **오브젝트 스토리지에서 복원 훈련** | 32테이블 3,148행 **전부 일치** |
| 크론 최소 환경(`env -i`) 실행 | 정상 |
| 업로드 실패 시 | 종료코드 1 (덤프만 되면 성공으로 치지 않는다) |
| DB 접속 정보 부재 시 | 종료코드 1, 즉시 중단 |

복원 훈련은 로컬 파일이 아니라 **오브젝트 스토리지에서 다시 내려받아** 수행했다.
해보지 않은 백업은 백업이 아니다.

### 남은 구멍 — 실패 알림

크론이 실패하면 `cron.log`에만 남고 아무도 보지 않는다. 스크립트에 알림 고리를
넣어뒀으나 **아직 연결되지 않았다.** 웹훅을 넣어야 작동한다.

```sh
printf '%s' 'https://hooks.slack.com/services/...' > ~/backup/slack-webhook
chmod 600 ~/backup/slack-webhook
```

이걸 채우기 전까지는 "백업이 돌고 있다"고 믿으면 안 된다. 주기적으로
`~/backup/cron.log` 또는 원격 목록의 최신 타임스탬프를 직접 확인해야 한다.

### 알아둘 것

백업은 앱의 오브젝트 스토리지 자격증명(`STORAGE_ACCESS_KEY`)을 그대로 쓴다. 별도
자격증명을 만들지 않아 운영 부담은 없지만, 그 키가 유출되면 백업까지 함께 노출된다.
분리하려면 OCI에서 백업 전용 키를 발급해 `~/backup/`에 두고 스크립트가 그것을 우선
읽도록 바꾼다.

OCI Object Storage는 aws-cli v2가 기본으로 붙이는 체크섬 헤더(aws-chunked)를 거부한다.
스크립트가 `AWS_REQUEST_CHECKSUM_CALCULATION=when_required`로 낮춰 두었다 —
`S3StorageClient.java`가 `chunkedEncodingEnabled(false)`를 쓰는 것과 같은 이유다.
