-- ============================================================================
-- WorkFlow AI - Database Schema (PostgreSQL DDL) — 생성물. 손으로 고치지 말 것.
--
-- 이 파일의 위치
--   빈 DB에 한 번에 실행할 수 있는 전체 스키마 스냅샷이다. 스키마의 소스가 아니라
--   결과물이다. 스키마를 바꾸려면 Flyway V파일을 추가한다.
--   → docs/decisions/2026-07-26-flyway-single-migration-path.md
--
-- 다시 만드는 법
--   1. 빈 pgvector/pgvector:pg17 컨테이너에 db/init/*.sql을 initdb로 태운다
--   2. baseline(20260721.1)보다 큰 V파일을 버전 순서대로 전부 적용한다
--   3. pg_dump --schema-only --schema=public --no-owner --no-privileges
--      (flyway_schema_history 계열 제외)
--   4. pg_dump가 --schema 지정 시 확장을 빼므로 CREATE EXTENSION vector를 넣고,
--      CREATE SCHEMA public에 IF NOT EXISTS를 붙인다 (아래 파일에 반영돼 있음)
--
--   운영 Supabase에서 직접 뜨지 않은 이유: 세션 풀러가 15 커넥션 한도라 pg_dump가
--   자주 실패한다. 위 절차는 자격 증명 없이 누구나 같은 결과를 얻는다.
--
-- 기준 시점과 운영과의 차이 (2026-07-29 실측)
--   마이그레이션 V20260729_6까지 반영. 같은 날 운영(ref: zzfcnbbzmbxzxptxghhq,
--   PostgreSQL 17.6)과 테이블·컬럼·제약·인덱스를 객체 단위로 대조했다. 남은 차이는 둘뿐이다.
--
--   1) assistant_messages — 운영에는 아직 있고 V20260729_6이 배포되면 사라진다.
--      이 파일은 그 배포 이후 상태다.
--   2) chk_projects_eval_status / chk_evaluation_scores_grade — 허용 값은 같고
--      표기만 다르다. pg_dump 왕복에서 ANY((ARRAY[...])::text[])가
--      ANY(ARRAY[(...)::text, ...])로 다시 렌더링되는 알려진 동작이다. 의미 차이 없음.
--      (V20260729_5의 정의 완전 일치 검증은 마이그레이션 체인으로 만든 DB를 대상으로
--       하므로 이 표기 차이에 걸리지 않는다.)
--
-- 왜 기계 생성으로 바꿨나
--   손으로 유지하던 이전 버전은 3일 만에 운영과 37개 객체가 어긋났고, 중복 컬럼 사고도
--   한 번 냈다. → docs/trouble-shooting/2026-07-27-schema-snapshot-duplicate-column.md
--
-- 요구 사항
--   PostgreSQL 13+ (운영은 17). document_chunks.embedding이 pgvector VECTOR(1024)를
--   쓰므로 자체 호스팅에서는 확장을 먼저 설치해야 한다
--   (예: apt install postgresql-17-pgvector).
-- ============================================================================
...
\restrict 1P6h6zF88gIRRni2d0nAjKfsdIEM4SmmWtWFBG2Fz5Y8SBcFglX6gvP1y6JB8UP
...
-- pg_dump --schema=public는 확장을 덤프에서 제외한다. document_chunks.embedding이
-- public.vector(1024)를 쓰므로 테이블 생성 전에 확장이 있어야 한다.
...
\unrestrict 1P6h6zF88gIRRni2d0nAjKfsdIEM4SmmWtWFBG2Fz5Y8SBcFglX6gvP1y6JB8UP
