...
7절 검증이 처음에는 객체 존재만 봤다. 그 방식으로는 가드가 잘못 건너뛴 것을 못 잡는다.
지금은 운영 기준값과 **정의**를 대조한다 — 컬럼 타입·NULL 허용, FK의 `ON DELETE` 동작
(`confdeltype`), varchar 길이 제한 유무, IDENTITY 여부, `eval_status` 기본값, `position`
기본값 부재까지. 하나라도 어긋나면 항목을 나열하고 예외를 던진다.
