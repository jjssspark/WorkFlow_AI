...
제약 추가 전에 중복을 해소하도록 고쳤다. `created_task_id`가 같은 행 중 **가장 오래된 것만
연결을 유지**하고 나머지는 `NULL`로 해제한다(업무 자체는 지우지 않는다). 해제한 건수는
`RAISE NOTICE`로 남긴다. 운영에서는 대상 행이 0건이라 아무 일도 일어나지 않는다.

7절에 **최종 상태 검증**도 추가했다. 이 마이그레이션은 `IF NOT EXISTS`류 가드에 의존하는데,
가드는 "없으면 만든다"만 보장하고 "있으면 최신으로 맞춘다"를 보장하지 않는다 —
이번 divergence를 만든 성질이 바로 그것이다. 조용히 건너뛴 채 성공으로 끝나지 않도록
기대 객체 21개의 존재를 확인하고 하나라도 없으면 예외를 던진다.
