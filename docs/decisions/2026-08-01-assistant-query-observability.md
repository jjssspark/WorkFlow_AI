...
- 날짜: 2026-08-01
- 상태: **구현 완료** (2026-08-02). 설계와 달라진 점은 맨 아래 "구현하며 달라진 것"에 적었다.
...
1. **로깅 설정 + 테스트** — 관측의 전제. 이것만으로도 기존 `logger.info`들이 살아난다.
   (2026-08-02 완료, PR #542)
2. **카운터 + ACL + 읽기 스크립트** — ACL은 공유 인프라라 별도 확인을 받는다.
   (2026-08-02 완료)

## 구현하며 달라진 것

설계 그대로 간 부분은 다시 적지 않는다. 판단이 바뀐 것만 남긴다.

### 1. ACL 에 `+hgetall` 을 함께 열었다

설계는 읽기를 `admin` 으로 하기로 했다. 그런데 **admin 비밀번호는 redis 컨테이너에만 있다**
(`docker-compose.prod.yml`: fastapi 컨테이너는 `REDIS_USERNAME=fastapi` 와 자기 비밀번호만
받는다). 그대로 두면 조회 스크립트를 돌리려고 비밀번호를 한 벌 더 실어 날라야 했다.

집계 카운터에는 숫자만 들어 있고 그 숫자를 쓴 주체가 fastapi 자신이라, 자기가 쓴 것을 자기가
읽게 두는 편이 비밀번호를 늘리는 것보다 낫다고 판단했다.

최종 선택자: `(~rag_stats:* +hincrby +expire +hgetall)`

`+multi`/`+exec` 는 일부러 넣지 않았다. redis-py 파이프라인 기본값(`transaction=True`)은
MULTI/EXEC 를 쓰는데 그 권한이 없으면 통째로 NOPERM 이 된다. 그래서 코드가 `transaction=False`
로 열고, 테스트가 그 계약을 강제한다(`tests/conftest.py` 의 `FakeStatsRedis.pipeline`).
...
설계는 169행 한 줄을 예상했다. 실제로는 세 곳이었고, 세 번째가 특히 나빴다.

| 위치 | 무엇이 어긋났나 |
| --- | --- |
| spring ACL 줄 | 2026-07-30 권한 보강(선택자 5개)을 따라가지 않은 고정 문자열 |
| fastapi ACL 줄 | 같은 이유 (`resetchannels`, `rag-jobs`, pub/sub 누락) |
| 롤백 대상 문자열 | `deploy-previous` → `deploy-last-good` 이름 변경을 따라가지 않음 |

세 번째는 `set -e` 아래에서 스크립트를 그 줄에서 **죽였다.** 즉 뒤따르는 롤백 순서 검사들이
통째로 도달조차 못 하고 있었다. 스크립트를 돌리면 아무 메시지 없이 exit 1 이 나온다.

그래서 두 가지를 바꿨다.

- ACL 검사를 줄 전체 고정 문자열에서 **성질별 검사**로 바꿨다. ① 거부 기본값(`resetkeys
  resetchannels -@all`, `~*`/`&*`/`+@all` 금지) ② 코드가 실제로 쓰는 키 묶음 존재.
  선택자를 추가해도 ①은 안 깨지고, ②는 기능이 바뀔 때만 같이 고치면 된다.
- **CI에 넣었다** (`.github/workflows/redis-acl-tests.yml`, 배포 게이트 `needs` 에도 추가).
  돌지 않는 테스트는 없는 것보다 나쁘다 — 있다고 믿게 만든다. 이 파일이 그 증거다.

`paths` 필터는 일부러 걸지 않았다. 이 스크립트는 ACL 템플릿뿐 아니라 compose, `.env.example`,
`deploy-oci.yml`, Lua, Java 발행기까지 걸쳐 단언한다. 좁게 걸면 이번에 발견된 것과 똑같은
사고(배포 워크플로 쪽 이름이 바뀌었는데 필터에 안 걸려 검사가 안 도는)가 그대로 재발한다.

### 4. 계측이 빠지는 것을 막는 테스트를 따로 뒀다

계측 사고는 계산이 틀려서가 아니라 **경로가 늘었을 때 호출이 안 붙어서** 난다. 그래서
필드 조건 테스트(`test_rag_stats.py`)와 별개로, 실제 검색 경로에서 카운터가 오르는지 보는
배선 테스트를 `test_retrieval_service.py` 에 뒀다. 계측 호출을 지우면 그 6건이 깨지는 것을
확인했다(지우고 실행 → 6 failed).

### 5. 테스트 격리를 autouse 로 강제했다

통계는 실패해도 예외를 삼키므로 "연결 실패로 어차피 안 쌓인다"에 기댈 수 없다. 개발자
로컬에 Redis 가 떠 있으면 **테스트가 운영과 같은 키에 카운트를 쌓아** 관측값을 오염시킨다.
같은 종류의 새어나감을 이미 두 번 겪었다(통합 테스트가 운영 DB 를 읽은 건, 회의록 분석
테스트가 로컬 캐시를 읽은 건). `tests/conftest.py` 의 `rag_stats_redis` 픽스처가 막는다.
