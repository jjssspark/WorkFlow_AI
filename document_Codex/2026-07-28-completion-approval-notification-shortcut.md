...
- 기존 회의·평가 알림 분기는 그대로 두고 `targetType=task` 분기에서 `COMPLETION_REQUESTED`만 별도 경로로 처리했다.
...
- 회의록 및 평가 알림의 기존 바로가기 동작은 유지했으며, 임의의 meeting 유형에 대한 토스트 회귀 테스트를 추가했다.

## 검증

- 집중 테스트: 3개 파일, 34개 테스트 통과
- 프로덕션 빌드: 성공
